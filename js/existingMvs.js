...
saveBtn.addEventListener("click", ()=>{
    ...
    const idNow = id + 1
    const createMoves = new NewMoves(titleInp, descInp, fromInp, toInp, dateInp, idNow)
    id = idNow
    // idClass(id)
    ...
    createEl(main,"div", "ex-cntn", `${idClass(id)}`,`cross${id}`, titleInp, descInp)
    console.log(existingMvs)
            const cross = document.querySelectorAll(".cross")
            cross[cross.length-1].addEventListener("click", (event)=>{
                classToggle("modal-delete-mask-hidden", "modal-delete-mask", modal_delete_hidden)
    ...
    pencil[pencil.length-1].addEventListener("click", ()=>{
        ...
        classToggle("editMaskForModal-hidden", "editMaskForModal", editMaskHidden)
